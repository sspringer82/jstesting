describe('number checker', function () {
  ...
  it('should not throw if 2 is provided', function () {
  ...
  it('should throw if "foo" is provided', function () {
    const throwWrapper = numberChecker.bind(null, 'foo');
    expect(throwWrapper).toThrow();
  });

  it('should throw an error if "foo" is provided', function () {
    const throwWrapper = numberChecker.bind(null, 'foo');
    expect(throwWrapper).toThrowError();
  });

  it('should throw an error with "not a number" if "foo" is provided', function () {
    const throwWrapper = numberChecker.bind(null, 'foo');
